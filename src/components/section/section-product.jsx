                    ...
                    <Col xl={12} lg={12} md={12} className="product-filter-item">
                        <ul>
                            {arrProduct && arrProduct.map((item, index) => {
                                return (<li key={index} className={item.isActive ? 'is-active-filter' : null} style={{ cursor: "pointer" }} onClick={() => { handleSelect(item) }} onKeyPress={() => { handleSelect(item) }} role={"button"} tabIndex={0}>
                                    {item.title}
                                </li>)
                            })}
                        </ul>
