                ...
                <div className={style['attribute-list']}>
                  <div className={style['attribute-left']}>
                    {item?.attributeList?.map((item) => {
                      return (
                        <div key={item.id} className={style['attribute-left-title']}>
                          {item.title}
                        </div>
                      )
                    })}
                  ...
                  <div className={style['attribute-right']}>
                    {item?.attributeList?.map((item) => {
                      return (
                        <div key={item.id} className={style['attribute-right-content']}>
...
const SectionParkingSystem = () => {
  ...
  const softwareContentList = t('smartParking.sectionParkingSystem.software.contentList', {
    returnObjects: true,
  })

  const hardwareItemList = t('smartParking.sectionParkingSystem.hardware.itemList', {
    returnObjects: true,
  })

  const appMapContent = t('smartParking.sectionParkingSystem.appMapContentList', {
    returnObjects: true,
  })
            ...
            <div className={style['content-container']}>
              <div className={style['content']}>
                ...
                <ul className={style['content-text']}>
                  {softwareContentList[`${seclection}`]?.content?.feature?.map((item) => {
              ...
              <div className={style['content']}>
                ...
                <ul className={style['content-text']}>
                  {softwareContentList[`${seclection}`]?.content?.desc?.map((item) => {
                    return <li key={item.id}>{item.text}</li>
                  })}
                </ul>
              </div>
                  ...
                  return (
                    <div key={id} className={style['appNmap-content']}>
                      ...
                      <div className={style['appNmap-right']}>
                        <div className={style['appNmap-right-title']}>{item.title}</div>
